...
export class WorldMap {
  ...
  constructor(containerId: string, countries: Country[], onSelectCountry: (countryId: string) => void) {
  ...
  private latLngToSvg(lat: number, lng: number): { x: number; y: number } {
    const x = (lng + 180) * (1000 / 360);
    const y = (90 - lat) * (500 / 180);
    return { x, y };
  }

  private createPinSvg(country: Country): string {
  ...
  private attachEvents(): void {
    if (!this.container) return;
    const pins = this.container.querySelectorAll<SVGGElement>('.map-pin');
    ...
    pins.forEach(pin => {
      ...
      pin.addEventListener('mouseenter', () => {
        if (tooltip) {
          ...
          const estYear = country.capital.establishedYear > 0 
          ...
          tooltip.innerHTML = `
            ...
            <div style="font-size: 0.75rem; color: #9CA3AF;">Established: ${estYear}</div>
