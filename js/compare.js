/**
 * Side-by-Side Country & Capital Comparison Component
 */

class CountryCompareView {
  constructor(containerId, countriesData) {
    this.container = document.getElementById(containerId);
    this.countries = countriesData;
    this.country1 = countriesData[0] ? countriesData[0].id : null;
    this.country2 = countriesData[1] ? countriesData[1].id : null;
    this.init();
  }
  ...
  render() {
    ...
    const c2 = this.countries.find(c => c.id === this.country2) || this.countries[1];
        ...
        <table class="compare-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>${c1.capital.establishedYear > 0 ? c1.capital.establishedYear + ' AD' : Math.abs(c1.capital.establishedYear) + ' BC'}</td>
              <td>${c2.capital.establishedYear > 0 ? c2.capital.establishedYear + ' AD' : Math.abs(c2.capital.establishedYear) + ' BC'}</td>
            </tr>
            <tr>
  ...
  attachEvents() {
    ...
    if (s1) {
      s1.addEventListener('change', (e) => {
        this.country1 = e.target.value;
        this.render();
      });
    }
    if (s2) {
      s2.addEventListener('change', (e) => {
        this.country2 = e.target.value;
        this.render();
      });
    }
  }
